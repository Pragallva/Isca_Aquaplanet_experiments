...
sigma_half[-1]=0.0001
p_sfc=data.variables['ps'][:].transpose(1,0,2)
times =data.variables['time'][:]

logging.debug(str(num)+'imported coordinates, height')

u = data.variables['ucomp'][:]* units.meter / (units.second)
logging.debug(str(num)+'imported u, %1d,%1d,%1d,%1d'%(u.shape))
ht = data.variables['zfull'][:]* units.meter 
logging.debug(str(num)+'imported ht, %1d,%1d,%1d,%1d'%(ht.shape))
temp = data.variables['temp'][:]* units.K
logging.debug(str(num)+'imported temp, %1d,%1d,%1d,%1d'%(temp.shape))
...
pres= pres_full[:,::-1,:,:].transpose(2,1,0,3)*units.pascals
logging.debug(str(num)+'imported pres, %1d,%1d,%1d,%1d'%(pres.shape))
logging.debug(str(num)+'imported pressure')
