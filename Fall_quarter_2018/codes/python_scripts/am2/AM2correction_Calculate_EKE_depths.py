...
log_directory='/project2/tas1/pragallva/Fall_quarter_2018/codes/shell_script/am2/log/'+'EKE'+edge+'_la'+land+'m_oc'+ocean+'m'
...
p_sfc=regrid3(v_var['ps'][:])  ## Pa
logging.debug('saved psfc '+"regrid:"+str(np.shape(p_sfc)))
...
v_comp    =regrid4(v_var['vcomp'][:,::-1,:,:])
logging.debug('saved v_comp '+"regrid:"+str(np.shape(v_comp)))
u_comp    =regrid4(v_var['ucomp'][:,::-1,:,:])
logging.debug('saved u_comp '+"regrid:"+str(np.shape(u_comp)))
Z         =regrid4(v_var['zfull'][:,::-1,:,:])
logging.debug('saved z '+"regrid:"+str(np.shape(Z)))
...
for p in range(len(sigma_full)):
    for la in range(len(lat)):
        for lo in range(len(lon)):
            ...
            Z_prime[:,p,la,lo]= FILTER(Z[:,p,la,lo])
...
EKE_dic      ={'Z_sq':R(Z_sq),'vert_Zsq':vert_Zsq,'EKE':R(EKE), 'vert_EKE':vert_EKE, 'EMF': R(EMF), 'vert_EMF':vert_EMF, 'lat':lat, 'lon':lon, 'sigma_full': sigma_full}
