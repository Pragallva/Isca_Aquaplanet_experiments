...
logging.basicConfig( filename = log_directory + exp_dir + num + var1+'_isen.log',level=logging.DEBUG, format='%(asctime)s %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p')
...
if ( (var1 != 'dtheta_dp') and (var1 != 'dtheta_dx') and (var1 != 'dtheta_dy') and (var1 != 'deltheta_delt') and (var1 != 'theta_dot')):
        ...
        VAR1 = np.gradient(theta, np.deg2rad(lon), axis=-1)* (1.0/(radius*np.cos(np.deg2rad(lat[None,None,:,None]))))* units.K/units.m
        logging.debug(str(num)+'calculated '+var1)

elif (var1 == 'dtheta_dy'):
        VAR1 = np.gradient(theta, np.deg2rad(lat), axis=-2)* (1.0/radius)* units.K/units.m
        logging.debug(str(num)+'calculated '+var1)
      
elif (var1 == 'deltheta_delt'):
        VAR1 = np.gradient(theta, dt, axis=0)*  units.K/units.second
        logging.debug(str(num)+'calculated '+var1) 

elif (var1 == 'theta_dot'):
        term1 = np.gradient(theta, dt, axis=0)
        logging.debug(str(num)+'calculated term1')

        dtheta_dp = np.gradient(theta, plevs, axis=1)
        logging.debug(str(num)+'calculated dtheta_dp')

        dtheta_dx = np.gradient(theta, np.deg2rad(lon), axis=-1)* (1.0/(radius*np.cos(np.deg2rad(lat[None,None,:,None]))))
        logging.debug(str(num)+'calculated dtheta_dx')

        dtheta_dy = np.gradient(theta, np.deg2rad(lat), axis=-2)* (1.0/radius)
        logging.debug(str(num)+'calculated dtheta_dy')

        u         = surface_pressure_correction(get_data('u')[0:last,...])
        logging.debug(str(num)+'calculated u')

        v         = surface_pressure_correction(get_data('v')[0:last,...])
        logging.debug(str(num)+'calculated v')

        w         = surface_pressure_correction(get_data('omega')[0:last,...])
        logging.debug(str(num)+'calculated w')      

        term2     = u*dtheta_dx; term3 = v*dtheta_dy; term4 = w*dtheta_dp
        logging.debug(str(num)+'calculated term 2,3,4')

        VAR1 = (term1+term2+term3+term4) *  units.K/units.second
        logging.debug(str(num)+'calculated '+var1)

else:
       logging.debug(str(num)+' Hmm! I cannot calculate '+var1+' for you. Sorry!')
       raise Exception(str(num)+' Hmm! I cannot calculate '+var1+' for you. Sorry!')
