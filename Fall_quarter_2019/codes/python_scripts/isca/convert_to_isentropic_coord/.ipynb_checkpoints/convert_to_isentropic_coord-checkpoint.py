...
def interp_to_isentrope(field1=height, field2=temp) 
   ...
   for t in range(len(time)):
	field1_isen = np.zeros((len(time),len(isen_levels),field1.shape[2],field1.shape[3]))
        ...
        isen1_levels1 = mcalc.isentropic_interpolation(isen_levels, plevs, temp[t,...], \
                                                  field1[t,...], field2[t,...], axis=1)
	logging.debug(str(num)+'interpolation done ht, T for '+str(t))

	field1_isen[t,...]     =  (isen1_levels1[0].magnitude)
	logging.debug(str(num)+' '+as_str(field1)+' isentrope for '+str(t))
	field2_isen[t,...]     =  (isen1_levels2[0].magnitude)
        logging.debug(str(num)+' '+as_str(field2)+' isentrope for '+str(t))
   
   return field1_isen, field2_isen 
