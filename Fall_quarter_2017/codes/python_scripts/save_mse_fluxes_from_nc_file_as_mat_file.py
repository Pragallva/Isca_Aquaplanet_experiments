...
#dirc=sys.argv
source=cwd+'/'
one_year=source+dirc[1]+'.nc'

destination=cwd+'/post_process_data/'+dirc[1]+'/'
...
coord_dic       ={"lat":lat,"lon":lon,"time":times,"p_sfc":p_sfc,"no_of_plevels":len(sigma_full),"sigma_full": sigma_full} 
...
# In[141]:
save(destination+"coordinates.mat"       ,coord_dic)
# np.save(destination+"time.mat"         ,time)
# np.save(destination+"EKE.mat"          ,EKE)
# save(destination+"fluxes_dic.mat"      ,fluxes_dic)
#save(destination+"rad_dic.hkl"         ,rad_dic)

logging.debug("successfully saved coordinates")
...
def MSE_total(m):
    ...
    vert_flux = zonal_f.sum(axis=-1)           # (year, month, lat)
...
save(destination+"zonal_decomposed_fluxes_dic.mat" ,zonal_decomposed_fluxes_dic)
...
save(destination+"zonal_radiation_dic.mat",zonal_radiation_dic)
