...
quarter     = '/project2/tas1/pragallva/Fall_quarter_2018/'
...
source   = quarter+'/exp_data/precip/'+exp_dir+'/'
one_year = source+exp_dir+num+'.nc'

destination = quarter+'/post_process_data/synoptic/'+exp_dir+num+'/'
...
def filterall(Y=ucomp, freq='high'):
    ...
    for p in range(len(sigma_full)):
      ...
      logging.debug('Filtering at'+str(sigma_full[p]/1000)+' sigma level for '+freq+' frequency')
    return prime             
