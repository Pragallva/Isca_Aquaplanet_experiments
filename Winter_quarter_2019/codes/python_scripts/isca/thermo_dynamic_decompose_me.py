...
source='/project2/tas1/pragallva/Fall_quarter_2018/exp_data/precip/'+filename+'/'
one_year=source+filename+num+'.nc'

destination='/project2/tas1/pragallva/Winter_quarter_2019/post_process_data/isca2/'+filename+num+'/'
...
def delta_v_mmc(m,vert=1):
    ...
    zonal_weight = weights.mean(axis=2).mean(axis=2).mean(axis=-1)    
    if vert==1:
        vert_flux = (zonal_m*delta_v/zonal_weight).sum(axis=2)  # (year, month, lat)    
        return vert_flux.mean(axis=0)              # (month, lat)--> yearly average
    else :
        return (zonal_m*delta_v/zonal_weight).mean(axis=0)
...
def delta_m_mmc(m,vert=1):
    ...
    zonal_weight = weights.mean(axis=2).mean(axis=2).mean(axis=-1)    
    if vert==1:
        vert_flux    = (zonal_v*delta_m/zonal_weight).sum(axis=2)  # (year, month, lat)    
        return vert_flux.mean(axis=0)              # (month, lat)--> yearly average
    else :
        return (zonal_v*delta_m/zonal_weight).mean(axis=0)
    
    
def delta_p_mmc(m,vert=1):
    v=v_comp
    M         = reshape(m)                     # (year, month,days, hour, plev, lat, lon)
    V         = reshape(v)  
    monthly_m = weighted(M).mean(axis=2).mean(axis=2)  # (year, month, days, hour, plev, lat, lon)
    ...
    zonal_v   = monthly_v.mean(axis=-1)
    zonal_weight = weights.mean(axis=2).mean(axis=2).mean(axis=-1)
    delta_p  =  zonal_weight-zonal_weight.mean(axis=1)[:,None,...]
    vert_flux = (-1*delta_p*zonal_m*zonal_v/zonal_weight**2).sum(axis=2)  # (year, month, lat)
    if vert==1:
        return vert_flux.mean(axis=0)              # (month, lat)--> yearly average
    else :
        return (-1*delta_p*zonal_m*zonal_v/zonal_weight**2).mean(axis=0)

def NL1_mmc(m,vert=1):
    v         = v_comp
    M         = reshape(m)                     # (year, month,days, hour, plev, lat, lon)
    V         = reshape(v)  
    monthly_m = weighted(M).mean(axis=2).mean(axis=2)  # (year, month, days, hour, plev, lat, lon)
    ...
    vert_flux    = (delta_m*delta_v/zonal_weight).sum(axis=2)  # (year, month, lat)    
    if vert==1:
        return vert_flux.mean(axis=0)              # (month, lat)--> yearly average
    else :
        return (delta_m*delta_v/zonal_weight).mean(axis=0)

def NL2_mmc(m,vert=1):
    v=v_comp
    M         = reshape(m)                     # (year, month,days, hour, plev, lat, lon)
    V         = reshape(v)  
    monthly_m = weighted(M).mean(axis=2).mean(axis=2)  # (year, month, days, hour, plev, lat, lon)
    ...
    vert_flux = (-1*delta_p*delta_v*zonal_m/zonal_weight**2).sum(axis=2)  # (year, month, lat)
    if vert==1:
        return vert_flux.mean(axis=0)              # (month, lat)--> yearly average
    else :
        return (-1*delta_p*delta_v*zonal_m/zonal_weight**2).mean(axis=0)
...
del_p_mmc           =delta_p_mmc(CpT+gZ+Lq)
nl1_mmc             =NL1_mmc(CpT+gZ+Lq)
nl2_mmc             =NL2_mmc(CpT+gZ+Lq)


del_m_mmc_vert           =delta_m_mmc(CpT+gZ+Lq,vert=0)
del_v_mmc_vert           =delta_v_mmc(CpT+gZ+Lq,vert=0)
del_p_mmc_vert           =delta_p_mmc(CpT+gZ+Lq,vert=0)
nl1_mmc_vert             =NL1_mmc(CpT+gZ+Lq,vert=0)
nl2_mmc_vert             =NL2_mmc(CpT+gZ+Lq,vert=0)

del_m_mmc_sensible           =delta_m_mmc(CpT)
del_v_mmc_sensible           =delta_v_mmc(CpT)
del_p_mmc_sensible           =delta_p_mmc(CpT)
nl1_mmc_sensible             =NL1_mmc(CpT)
nl2_mmc_sensible             =NL2_mmc(CpT)

del_m_mmc_sensible_vert           =delta_m_mmc(CpT,vert=0)
del_v_mmc_sensible_vert           =delta_v_mmc(CpT,vert=0)
del_p_mmc_sensible_vert           =delta_p_mmc(CpT,vert=0)
nl1_mmc_sensible_vert             =NL1_mmc(CpT,vert=0)
nl2_mmc_sensible_vert             =NL2_mmc(CpT,vert=0)

del_m_mmc_moist           =delta_m_mmc(Lq)
del_v_mmc_moist           =delta_v_mmc(Lq)
del_p_mmc_moist           =delta_p_mmc(Lq)
nl1_mmc_moist             =NL1_mmc(Lq)
nl2_mmc_moist             =NL2_mmc(Lq)

del_m_mmc_moist_vert           =delta_m_mmc(Lq,vert=0)
del_v_mmc_moist_vert           =delta_v_mmc(Lq,vert=0)
del_p_mmc_moist_vert           =delta_p_mmc(Lq,vert=0)
nl1_mmc_moist_vert             =NL1_mmc(Lq,vert=0)
nl2_mmc_moist_vert             =NL2_mmc(Lq,vert=0)

del_m_mmc_pot           =delta_m_mmc(gZ)
del_v_mmc_pot           =delta_v_mmc(gZ)
del_p_mmc_pot           =delta_p_mmc(gZ)
nl1_mmc_pot             =NL1_mmc(gZ)
nl2_mmc_pot             =NL2_mmc(gZ)

del_m_mmc_pot_vert           =delta_m_mmc(gZ,vert=0)
del_v_mmc_pot_vert           =delta_v_mmc(gZ,vert=0)
del_p_mmc_pot_vert           =delta_p_mmc(gZ,vert=0)
nl1_mmc_pot_vert             =NL1_mmc(gZ,vert=0)
nl2_mmc_pot_vert             =NL2_mmc(gZ,vert=0)


MMC_thermo_dynamic_dic   ={"MM_flux":MM_flux,"del_m_mmc":del_m_mmc, "del_v_mmc":del_v_mmc , "del_p_mmc":del_p_mmc, "nl1_mmc":nl1_mmc, "nl2_mmc":nl2_mmc, "del_m_mmc_sensible":del_m_mmc_sensible, "del_v_mmc_sensible":del_v_mmc_sensible, "del_p_mmc_sensible":del_p_mmc_sensible,   "del_v_mmc_moist":del_v_mmc_moist, "del_m_mmc_moist":del_m_mmc_moist, "del_p_mmc_moist":del_p_mmc_moist, "del_v_mmc_pot":del_v_mmc_pot,"del_m_mmc_pot":del_m_mmc_pot, "del_p_mmc_pot":del_p_mmc_pot, "nl1_mmc":nl1_mmc, "nl2_mmc":nl2_mmc, "nl1_mmc_sensible":nl1_mmc_sensible, "nl2_mmc_sensible":nl2_mmc_sensible, "nl1_mmc_moist":nl1_mmc_moist, "nl2_mmc_sensible":nl2_mmc_moist, "nl1_mmc_pot":nl1_mmc_pot, "nl2_mmc_pot":nl2_mmc_pot }

save(destination+"MMC_thermo_dynamic_dic.hkl" ,MMC_thermo_dynamic_dic)
logging.debug("loaded MMC_thermo_dynamic dictionary")

MMC_thermo_dynamic_dic_vert   ={"MM_flux":MM_flux_vert,"del_m_mmc":del_m_mmc_vert, "del_v_mmc":del_v_mmc_vert , "del_p_mmc":del_p_mmc_vert, "nl1_mmc":nl1_mmc_vert, "nl2_mmc":nl2_mmc_vert, "del_m_mmc_sensible":del_m_mmc_sensible_vert, "del_v_mmc_sensible":del_v_mmc_sensible_vert, "del_p_mmc_sensible":del_p_mmc_sensible_vert,   "del_v_mmc_moist":del_v_mmc_moist_vert, "del_m_mmc_moist":del_m_mmc_moist_vert, "del_p_mmc_moist":del_p_mmc_moist_vert, "del_v_mmc_pot":del_v_mmc_pot_vert,"del_m_mmc_pot":del_m_mmc_pot_vert, "del_p_mmc_pot":del_p_mmc_pot_vert, "nl1_mmc":nl1_mmc_vert, "nl2_mmc":nl2_mmc_vert, "nl1_mmc_sensible":nl1_mmc_sensible_vert, "nl2_mmc_sensible":nl2_mmc_sensible_vert, "nl1_mmc_moist":nl1_mmc_moist_vert, "nl2_mmc_sensible":nl2_mmc_moist_vert, "nl1_mmc_pot":nl1_mmc_pot_vert, "nl2_mmc_pot":nl2_mmc_pot_vert }
