...
log_directory='/project2/tas1/pragallva/Winter_quarter_2019/codes/shell_script/greyT85/log/'+'baroclinic'+filename
...
destination='/project2/tas1/pragallva/Winter_quarter_2019/post_process_data/greyT85/data_in_pres_coord/'+filename+num+'/'+duration+'/'
make_sure_path_exists(destination);

data = nc.Dataset(one_year,'r')

logging.debug(duration+'---'+str(num)+'...........interpolation_from_sigma_to_pres_coord.py.............')
...
times = data.variables['time'][ll:ul]

logging.debug(str(num)+'imported coordinates, height')

u = data.variables['ucomp'][ll:ul,...]   * units.meter / (units.second)
logging.debug(str(num)+'imported u')
ht = data.variables['height'][ll:ul,...] * units.meter 
logging.debug(str(num)+'imported ht')
temp = data.variables['temp'][ll:ul,...] * units.K
logging.debug(str(num)+'imported temp')
pres = data.variables['pres_full'][ll:ul,...] * units.pascals
