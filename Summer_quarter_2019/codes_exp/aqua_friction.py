...
experiment_name=str(dirc[1])+'x_stress'
baseexp = Experiment(experiment_name, overwrite_data=False)

baseexp.inputfiles = ['/project2/tas1/pragallva/isca_sim/Isca/input/ozone_1990.nc'] 
...
diag_spinup.add_field('atmosphere', 'dt_ug_diffusion',time_avg=True, files=['atmos_4xdaily'])
diag_spinup.add_field('atmosphere', 'dt_vg_diffusion',time_avg=True, files=['atmos_4xdaily'])
diag_spinup.add_field('atmosphere', 'dt_tg_diffusion',time_avg=True, files=['atmos_4xdaily'])
diag_spinup.add_field('atmosphere', 'dt_qg_diffusion',time_avg=True, files=['atmos_4xdaily'])
...
baseexp.namelist['mixed_layer_nml']['depth'] = 5            #float(dirc[1])
...
baseexp.namelist['surface_flux_nml']['stress_factor'] = float(dirc[1])
