...
log_directory='/project2/tas1/pragallva/Fall_quarter_2018/codes/shell_script/am2/log/'+'baroclinic'+edge+'_la'+land+'m_oc'+ocean+'m'
...
destination='/project2/tas1/pragallva/Fall_quarter_2018/post_process_data/am2/data_in_pres_coord/'+'HC'+edge+'_la'+land+'m_oc'+ocean+'m'+num+'/'
...
lat=nc.Dataset(one_year,'r').variables['lat'][:]
lon   = data.variables['lon'][:]
sigma_full  = data.variables['pfull'][::-1] # sigma_coord
sigma_half = data.variables['phalf'][::-1]
sigma_half[-1]=0.0001
...
def regrid3(Y):
    ...
    y=np.reshape(np.array(y), (30*3,1460,len(lon)))    
    return y
...
def regrid4(Y):
    ...
    y=np.reshape( np.array(y), (30*3,1460,24,len(lon)) )    
    return y

p_sfc=regrid3(v_var['ps'][:])


u = regrid4(data.variables['ucomp'][:])   * units.meter / (units.second)
logging.debug(str(num)+'imported u')
ht = regrid4(data.variables['zfull'][:]) * units.meter 
logging.debug(str(num)+'imported ht')
temp = regrid4(data.variables['temp'][:]) * units.K
...
pres= regrid4(pres_full[:,::-1,:,:])*units.pascals
logging.debug(str(num)+'imported pressure')
...
def reshape(x): # average over years
    x=x[0:30*12*4,...].reshape(no_of_years,no_of_months,days,hours,len(plevs),len(lat),len(lon))
