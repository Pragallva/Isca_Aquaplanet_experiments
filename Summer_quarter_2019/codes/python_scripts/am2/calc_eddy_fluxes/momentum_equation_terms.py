...
source   = quarter+'/exp_data/am2_repeat/data_in_pres_coord/'+exp_dir+'/'
one_year = source+exp_dir+num+'.nc'

destination = quarter+'/post_process_data/am2_repeat/data_in_pres_coord/avged_over'+str(no_of_days)+'days/'+exp_dir+num+'/'
make_sure_path_exists(destination);

v_variables = nc.Dataset(one_year,'r')
v_var=v_variables.variables

logging.debug('....Extract_ncfile_save_fluxes_radiation.py....'+str(num)+' averaged over '+str(no_of_days))
...
def get_data(variable='ucomp', f = one_year):
    ncfile = f
...
def zonal_mean_time_mean_flux(a=ucomp, b=vcomp):    
...
def zonal_mean_time_eddy_flux(a=ucomp, b=vcomp):
    A                       = reshape(a)                     # (year, month,days, hour, plev, lat, lon)
    B                       = reshape(b)
    zonal_A                 = zonal_mean(A)
    zonal_B                 = zonal_mean(B)
...
def zonal_eddy_time_eddy_flux(a=ucomp, b=vcomp):
    A                       = reshape(a)
    B                       = reshape(b)
...
def zonal_eddy_time_mean_flux(a=ucomp, b=vcomp): #### This is similar to stationary eddies as defined in our paper
    A                       = reshape(a)
    B                       = reshape(b)
    zonal_eddy_A            = zonal_eddy(A)
    zonal_eddy_B            = zonal_eddy(B)
...
def zonal_mean_flux(a=ucomp, b=vcomp):
    A                       = reshape(a)
    B                       = reshape(b)
...
def zonal_eddy_flux(a=ucomp, b=vcomp):
    A                       = reshape(a)
    B                       = reshape(b)
    zonal_eddy_A            = zonal_eddy(A)
    zonal_eddy_B            = zonal_eddy(B)
...
def zonal_eddy_flux2(a=ucomp, b=vcomp):  ### mathematically this should be same as zonal_eddy_flux()
    x1                      = zonal_eddy_time_mean_flux(a,b)
    x2                      = zonal_eddy_time_eddy_flux(a,b)
    return x+x2

def time_eddy_flux(a=ucomp, b=vcomp):    #### This is similar to transient eddies as defined in our paper
    A                       = reshape(a)
    B                       = reshape(b)
...
def time_eddy_flux2(a=ucomp, b=vcomp):  ### mathematically this should be same as zonal_eddy_flux()
    x1                      = zonal_mean_time_eddy_flux(a,b)
    x2                      = zonal_eddy_time_eddy_flux(a,b)
    return x+x2

def total_flux(a=ucomp, b=vcomp):
    A                       = reshape(a)
    B                       = reshape(b)
    flux                    = A*B
    mean_flux               = zonal_mean(time_mean(flux))
...
def vort(a=ucomp,ax=2):
   cos_phi      =  np.cos(np.deg2rad(lat))[None, None, :, None]
   vorticity    = -np.gradient(a*cos_phi, np.deg2rad(lat), axis = ax)/(radius*cos_phi)
   return vorticity


def R(y):
    y1= zonal_mean(time_mean(reshape(y)))
    return np.squeeze(y1)


pres_ex         = np.append(pres,0)
dp_by_g         = (pres_ex[1:]-pres_ex[:-1])/g

mv_meridional   =  zonal_mean_time_mean_flux( a=vcomp, b=MSE )
raw_data_dic    =  {"MSE":R(MSE),'v':R(vcomp),'CpT':R(CpT),'Lq':R(Lq),'gZ':R(gZ), 'mv':mv_meridional, 'dp_by_g':dp_by_g }
...
zeta_mean          = np.squeeze( zonal_mean( time_mean( reshape(vort())  )))
...
sin_phi          = (np.sin(np.deg2rad(lat)))[None,None,:,None]
fv               = reshape(2*rot_rate*sin_phi*(vcomp))
...
EMF_teddy         = time_eddy_flux(a=ucomp, b=vcomp)
logging.debug(str(num)+'calculated EMF5')

EMF_zmean_teddy  = zonal_mean_time_eddy_flux(a=ucomp, b=vcomp)
logging.debug(str(num)+'calculated EMF6')
...
def div_EMF(X):
  ...
  div =  (1/(radius*cos_phi**2)) * (np.gradient (X*cos_phi**2, np.deg2rad(lat), axis=-1 ))
  return div
...
div_EMF_teddy         = div_EMF(time_eddy_flux(a=ucomp, b=vcomp))
logging.debug(str(num)+'calculated EMF5')

div_EMF_zmean_teddy  = div_EMF( zonal_mean_time_eddy_flux(a=ucomp, b=vcomp))
logging.debug(str(num)+'calculated EMF6')


divEMF = {'div_EMF_zeddy': div_EMF_zeddy, 'div_EMF_zeddy2': div_EMF_zeddy2, 'div_EMF_teddy':div_EMF_teddy, \
...
u_w_teddy       =  time_eddy_flux( a=ucomp, b=omega )
logging.debug(str(num)+'calculated uw5')

u_w_zmean_teddy =  zonal_eddy_time_eddy_flux( a=ucomp, b=omega )
logging.debug(str(num)+'calculated uw6')
...
du_w_teddy_dp       =  ugrad_dp(a=u_w_teddy, ax=1)
logging.debug(str(num)+'calculated duw5_dp')

du_w_zmean_teddy_dp =  ugrad_dp(a=u_w_zmean_teddy, ax=1)
logging.debug(str(num)+'calculated duw6_dp')
...
save(destination+"raw_data.hkl", raw_data_dic)
...
logging.debug(str(num)+"-Finished- time taken = "+str(end-start) )  
